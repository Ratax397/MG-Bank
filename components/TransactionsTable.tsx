    ...
    <Table>
        ...
        <TableBody>
            {transactions.map((t: Transaction)=>{
              ...
              return(
                <TableRow key={t.id} className={`${isDebit || amount[0] === '-' ? 'bg-[#FFFBFA]' : 'bg-[#F6FEF9]'} !over:bg-none !border-b`}>
...
// Fonction pour normaliser les catégories Plaid vers vos styles
const normalizeCategory = (cat: string) => {
  // Convertir FOOD_AND_DRINK → Food And Drink
  const normalized = cat
    .toLowerCase()
    .replace(/_/g, ' ')
    .replace(/\b\w/g, (char) => char.toUpperCase());
  
  // Mapper les catégories Plaid vers les styles définis
...
const CategoryBadge = ({category}: CategoryBadgeProps) => {
  ...
  const {
    ...
    textColor,
    chipBackgroundColor
  } = transactionCategoryStyles[mappedCategory as keyof typeof transactionCategoryStyles] || transactionCategoryStyles.default;
